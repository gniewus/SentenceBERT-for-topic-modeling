...
df = pd.read_csv("./dash/ArticlesTopicsPageViews.csv")
...
def base_plot(result):
    ...
    fig = px.scatter(result, y="count_in_group", x="topic_label", hover_name="title", hover_data={"date": True, 'topic_label': True},  # ["created_at"],
...
def get_top_topics_table_data(df, start, end):
    ...
    tmp = df.loc[df.topic_label.isin(topic_labels)].groupby(["topic_label", 'topic_desc', "topic_size"]).sum()\
            .sort_values(by=["PageViews"], ascending=False)\
            .reset_index()[["topic_label", "topic_desc", "topic_size", "PageViews"]]\
            .head(20).copy()
                    ...
                    html.Div([
                        dbc.Card(
                            [
                                dbc.CardBody([
                                    html.H1(
                                        children='Topic Thunder exploration dashboard', id="top-title"),
                                    html.P(
                                        children='''Here you can explore most popular topics over a period of 7 months. 
                                    Pick a time range of interest in the top-right corner and enjoy.''', id="desc")
                                ])
                            ])
                    ])
                ], width=8),
                            ...
                            dbc.CardBody([
                                ...
                                dash_table.DataTable(
                                    ...
                                    columns=[{'id': "topic_label", 'name': "topic_label", 'hideable': True},
                                             {"id": "topic_desc", "name": "Topic Description"}, 
                                             {"id": "topic_size", "name": "Total size of topic", "type": "numeric"},
                                             {"id": "PageViews", "name": "Page Views", "type": "numeric","format":dict(specifier=',.0f', locale=dict(separate_4digits=False))},
...
@app.callback(Output('image_wc', 'src'), [Input('image_wc', 'id'), Input("graph3", "hoverData"),Input("top-topics","selected_rows")], State('top-topics', 'data'))
def make_image(inp, value,selected_rows, data_top_topics):
    ...
    if value:
        print(value['points'])
        topic_label = value['points'][0]["customdata"][1]
        tmp = df[df.topic_label == topic_label].head(1)["topic_desc"].values
        tmp = tmp[0].replace("['", "").replace("']", '').split(', ')
    ...
    if selected_rows:
        ...
        tmp = df[df.topic_label == selected_topic_label].head(1)["topic_desc"].values
        tmp = tmp[0].replace("['", "").replace("']", '').split(', ')
        words_freq = dict(zip(tmp, list(range(1, len(tmp), 1))[::-1]))
        plot_wordcloud(d=words_freq).save(img, format='PNG')
